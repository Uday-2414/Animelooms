...
import { determineDashboardType, USER_TYPES } from '../utils/userType'
...
export default function Home() {
  ...
  // Determine user state dynamically via centralized helper
  const dashboardType = useMemo(() => {
    return determineDashboardType({
      user,
      progressList,
      authLoading,
      progressLoading,
    })
  }, [user, progressList, authLoading, progressLoading])

  // Renders the standard hero landing page for anonymous / guest users
  ...
  return (
    <>
      <SEO
        ...
        pathname="/"
      />
      {dashboardType === USER_TYPES.LOADING ? (
        <div className="py-20">
          <LoadingState message="Loading dashboard..." />
        </div>
      ) : dashboardType === USER_TYPES.NEW_USER ? (
        <NewUserDashboard
          user={user}
        ...
        />
      ) : dashboardType === USER_TYPES.RETURNING_USER ? (
