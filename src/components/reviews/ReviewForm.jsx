...
export default function ReviewForm({ initialData = null, onSubmit, onCancel, isLoading }) {
  ...
  const handleSubmit = (e) => {
    ...
    setError('');
    // Sanitization is handled server-side / by React rendering text naturally
    onSubmit({ rating, title: title.trim(), review: review.trim() });
  };

  return (
    <div className="bg-surface-card border border-white/10 rounded-2xl p-6 shadow-2xl w-full max-w-lg mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-lg font-bold text-white font-ui">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-400 text-sm p-3 rounded-lg font-ui">
            {error}
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="ml-3 text-lg font-black text-white font-ui w-8 text-center">
              {hoverRating || rating || '-'}
            </span>
        ...
        {/* Title Input */}
        <div className="space-y-2">
          <label htmlFor="review-title" className="block text-sm font-bold text-gray-300 font-ui uppercase tracking-wider">
        ...
        {/* Review Body Input */}
        <div className="space-y-2">
          <label htmlFor="review-body" className="block text-sm font-bold text-gray-300 font-ui uppercase tracking-wider">
            Review (Optional)
          </label>
          <textarea
            id="review-body"
            ...
            placeholder="What did you think of this anime? Did it meet your expectations?"
            rows={5}
            maxLength={2000}
          ...
          />
          <div className="text-right text-xs text-gray-500 font-ui font-semibold">
            {review.length}/2000
          </div>
        </div>
